  ...
  const rows = useMemo(() => {
    return items.map((item) => (
      <Table.Tr key={item.id}>
        ...
        <Table.Td>
          <Text fw={600}>{Number(item.amount).toLocaleString()}원</Text>
        </Table.Td>
        <Table.Td>
        ...
        <Table.Td>
          <Group gap="xs" justify="flex-end" wrap="nowrap">
            <Button size="xs" variant="light" color="gray" onClick={() => void openEdit(item)}>
              열기
            </Button>
            {item.status === "draft" && (
              <ActionIcon variant="subtle" color="gray" onClick={() => void action(item, "submit")} aria-label="submit">
                <IconSend size={16} />
              </ActionIcon>
            )}
            {item.status === "submitted" && (
              <>
                <ActionIcon variant="subtle" color="blue" onClick={() => void action(item, "approve")} aria-label="approve">
                  <IconCheck size={16} />
                </ActionIcon>
                <ActionIcon variant="subtle" color="red" onClick={() => void action(item, "reject")} aria-label="reject">
                  <IconX size={16} />
                </ActionIcon>
              </>
            )}
            {item.status === "approved" && (
              <ActionIcon variant="subtle" color="green" onClick={() => void action(item, "pay")} aria-label="pay">
                <IconCash size={16} />
              </ActionIcon>
            )}
  ...
  return (
    <Box p="md">
      ...
      <Paper className="app-surface" p="lg" radius="md">
        ...
        {!items.length && !loading && <Text size="sm" c="dimmed">등록된 경비가 없습니다.</Text>}
      </Paper>
      ...
      <Modal opened={opened} onClose={() => setOpened(false)} title={editing ? "경비 상세" : "경비 등록"} centered size="lg">
        <Stack gap="sm">
          ...
          <Group grow>
            ...
            <DateInput label="사용일" value={spentAt} onChange={setSpentAt} valueFormat="YYYY-MM-DD" required />
          </Group>
